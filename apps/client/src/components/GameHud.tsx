...
export function GameHud(): JSX.Element {
  ...
  const discovered = snapshot.observedRelations.filter((edge) => edge.predator === speciesId).length;
  const total = foodsFor(speciesId).length;
  const skillRemaining = Math.max(0, (player?.skillReadyAt ?? 0) - now);
  const eatRemaining = Math.max(0, (player?.eatReadyAt ?? 0) - now);
    ...
    <div className="hud-layer">
      <div className="hud-top-left">
        ...
        <div className="hunger-bar">
          <span>배고픔</span>
          <div><i style={{ width: `${player?.hunger ?? 100}%` }} /></div>
        </div>
      </div>

      <div className="round-timer">
        <small>{snapshot.roundNumber ? `${snapshot.roundNumber}판` : "생태계 실험"}</small>
        <strong>{formatTime(snapshot.timeRemainingMs)}</strong>
        {snapshot.shrinkStage > 0 && <span>⚠️ 서식 공간 축소 {snapshot.shrinkStage}/2</span>}
      </div>

      <div className="hud-top-right">
        <div className="discovery-chip">🔗 내 관계 <strong>{discovered}/{total}</strong></div>
        <div className="score-chip">✨ {player?.score.toFixed(1) ?? "0.0"}</div>
      </div>

      <VirtualJoystick />

      <div className="action-buttons">
        <button className="action-button skill-button" style={cooldownStyle(skillRemaining / skillCooldown)} onPointerDown={(event) => { event.preventDefault(); useSkill(); }} disabled={skillRemaining > 0 || player?.status !== "active"}>
        ...
        <button className="action-button eat-button" style={cooldownStyle(eatRemaining / EAT_COOLDOWN_MS)} onPointerDown={(event) => { event.preventDefault(); eatNearest(); }} disabled={player?.status !== "active" || (player?.wrongUntil ?? 0) > now || eatRemaining > 0}>
      ...
      {(player?.wrongUntil ?? 0) > now && <div className="center-banner sick-banner"><span>😵</span><strong>배탈!</strong><small>잠깐 움직일 수 없어요</small></div>}
      {snapshot.paused && <div className="pause-curtain"><span>⏸️</span><strong>잠시 멈춤</strong><small>선생님의 안내를 들어보세요</small></div>}
    </div>
  );
}
