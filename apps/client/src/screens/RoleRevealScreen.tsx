      ...
      <section className="role-card">
        ...
        <div className="relation-panels">
          <div className="relation-panel food-panel">
            ...
            <div>{foods.map((id) => <span key={id}><PixelSpeciesIcon speciesId={id} /> {SPECIES[id].name}</span>)}</div>
          </div>
          <div className="relation-panel danger-panel">
            <strong>나를 먹을 수 있어요</strong>
            <div>{predators.length ? predators.map((id) => <span key={id}><PixelSpeciesIcon speciesId={id} /> {SPECIES[id].name}</span>) : <span>상위 포식자예요</span>}</div>
          </div>
        </div>
