...
import { GAME_MODE_IDS, PLAYABLE_SPECIES, SPECIES, isPlayableSpeciesId, modeConfig, type GameModeId, type SpeciesId, type TrophicLevel } from "@feed-chain/shared";
...
const ROLE_LEVELS: readonly { id: TrophicLevel; label: string }[] = [
  { id: "apex", label: "최상위 포식자" },
  { id: "secondary", label: "2차 소비자" },
  { id: "primary", label: "1차 소비자" },
];
    ...
    <main className="teacher-lobby-screen">
      ...
      <section className="teacher-lobby-grid">
        ...
        <div className="teacher-control-card">
          <div className="control-section roster-section">
            ...
            <div className="teacher-roster">
              {snapshot.players.length ? snapshot.players.map((player) => {
                ...
                return (
                  <button key={player.id} type="button" className={`${selectedPlayerId === player.id ? "selected" : ""} ${player.connected ? "" : "offline"}`} onClick={() => setSelectedPlayerId(player.id)}>
          ...
          {selectedPlayer && snapshot.phase !== "role_reveal" && (
            <div className="control-section role-section">
              <div className="section-heading"><div><span>03</span><strong>{selectedPlayer.name} 역할 지정</strong></div><small>교사에게만 보임</small></div>
              <div className="role-level-groups">
                {ROLE_LEVELS.map((level) => {
                  const roles = PLAYABLE_SPECIES.filter((species) => species.level === level.id && playableForMode.has(species.id));
                  if (!roles.length) return null;
                  return (
                    <div key={level.id} className="role-level-group">
                      <small>{level.label}</small>
                      <div>{roles.map((species) => {
                        const active = (assignmentById.get(selectedPlayer.id) ?? selectedPlayer.species) === species.id;
                        return <button key={species.id} type="button" className={active ? "active" : ""} onClick={() => sendTeacherCommand({ action: "set_role", playerId: selectedPlayer.id, species: species.id })}><PixelSpeciesIcon speciesId={species.id} /><span>{species.name}</span></button>;
                      })}</div>
                    </div>
                  );
                })}
          ...
          <div className="control-section mode-section">
            <div className="section-heading"><div><span>{selectedPlayer ? "04" : "03"}</span><strong>게임 모드 선택</strong></div></div>
